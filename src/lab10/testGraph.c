...
void failed(char *s) { printf("Testul %s NU a fost trecut!\n", s); }
...
int testDFS(TGraphL **gl, float score) {
  List *path = dfs(*gl, 0);
...
void buildGraphsFromFile(TGraphL **gl) {
  ...
  int v1, v2;
  if (scanf("%d %d", &numOfVertices, &numOfEdges) != 2) {
